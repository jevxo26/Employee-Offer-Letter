...
// ─── Tab content panels ───────────────────────────────────────────────────────
function SettingsTab({ docSettings, setDocSettings }) {
  const numericSetter = (key, min, max) => e =>
    setDocSettings(p => ({ ...p, [key]: Math.min(max, Math.max(min, parseInt(e.target.value) || min)) }));
...
function SecondPartyTab({ secondParty, setSecondParty }) {
  const set = key => e => setSecondParty(p => ({ ...p, [key]: e.target.value }));
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            {['Father', 'Mother', 'Husband', 'Guardian'].map(r => <option key={r}>{r}</option>)}
          </select>
        </div>
      </div>
...
function FirstPartyTab({ firstParty, setFirstParty }) {
  const set = key => e => setFirstParty(p => ({ ...p, [key]: e.target.value }));
  return (
    <div className="space-y-4">
