    ...
    activeStep === 1 ? (
      ...
      isCSP ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="md:col-span-2">
            <h3 className="text-[#0F172A] font-bold text-base">
              Country Sales Partner information
            </h3>
            <p className="text-[#64748B] text-xs mt-1">
              This is the contracting party with JEVXO.
            </p>
          </div>
          ...
          <Field label="Partner Phone *">
            <Input
              ...
              onChange={(e) =>
                setSecondParty((p) => ({ ...p, mobileNumber: e.target.value }))
              }
            />
          </Field>
          <div className="md:col-span-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="md:col-span-2">
            ...
            <p className="text-[#64748B] text-xs mt-1">
              The Country Sales Partner contracts with the Sales Agent. JEVXO
          ...
          <Field label="Country Sales Partner Name *">
            <Input
              ...
              onChange={(e) => setPartner({ fullName: e.target.value })}
            />
          </Field>
          <Field label="Partner Agreement Reference *">
            <Input
              value={docSettings.partnerAgreementRef || ""}
              onChange={(e) =>
                setSettings({ partnerAgreementRef: e.target.value })
              }
            />
          </Field>
          <Field label="Partner Email *">
          ...
          <Field label="Sales Agent ID">
            <Input
              value={secondParty.salesPartnerId || ""}
              readOnly
            />
          </Field>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <Field
          label={
            isCSP ? "Country / Territory *" : "Assigned Territory / Region *"
          }
        >
          <Input
            value={docSettings.territory || ""}
            onChange={(e) => setSettings({ territory: e.target.value })}
          />
        </Field>
        <Field label="Governing Jurisdiction *">
          <Input
            value={docSettings.governingJurisdiction || "Bangladesh"}
            onChange={(e) =>
              setSettings({ governingJurisdiction: e.target.value })
            }
          />
        </Field>
        {isCSP ? (
          <div className="md:col-span-2">
            <Field label="Exclusivity *">
              <select
                ...
                onChange={(e) =>
                  setSettings({ isExclusive: e.target.value === "true" })
                }
          ...
          <>
            <Field label="Reporting Structure *">
              <Input
                value={
                  docSettings.reportingStructure || "Country Sales Partner"
                }
                onChange={(e) =>
                  setSettings({ reportingStructure: e.target.value })
                }
              />
            </Field>
            <Field label="Partner Address">
              <Input
                ...
                onChange={(e) => setPartner({ address: e.target.value })}
              />
            </Field>
          </>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <SliderControl
          label={isCSP ? "Base Commission" : "Sales Commission"}
          value={docSettings.baseCommissionRate ?? 10}
          min={1}
          max={100}
          onChange={(value) => setSettings({ baseCommissionRate: value })}
        />
        <SliderControl
          label="Recurring Commission"
          value={docSettings.recurringCommissionRate ?? (isCSP ? 12 : 10)}
          min={1}
          max={100}
          onChange={(value) => setSettings({ recurringCommissionRate: value })}
        />
        {isCSP && (
          <SliderControl
            label="Override Commission"
            value={docSettings.overrideCommissionRate ?? 10}
            min={1}
            max={100}
            onChange={(value) => setSettings({ overrideCommissionRate: value })}
          />
        )}
        <SliderControl
          label="Payment Window"
          value={docSettings.paymentDays ?? 14}
          suffix=" Days"
          min={1}
          max={90}
          onChange={(value) => setSettings({ paymentDays: value })}
        />
        <Field label="Payment Currency *">
          <select value={docSettings.paymentCurrency || "BDT"} onChange={(e) => setSettings({ paymentCurrency: e.target.value })} className="w-full bg-[#F8FAFC] border border-[#DBEAFE] rounded-xl py-3 px-4 text-sm">
            {["BDT", "USD", "EUR", "GBP", "INR", "PKR", "AED", "SAR", "CAD", "AUD"].map((currency) => <option key={currency} value={currency}>{currency}</option>)}
          </select>
        </Field>
        ...
        <SliderControl
          label="Notice Period"
          value={Number(docSettings.noticePeriodSales) || 30}
          suffix=" Days"
          min={1}
          max={90}
          onChange={(value) =>
            setSettings({ noticePeriodSales: String(value) })
          }
        />
      </div>
    ) : (
